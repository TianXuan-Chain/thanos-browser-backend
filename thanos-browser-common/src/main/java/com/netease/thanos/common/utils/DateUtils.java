...
public class DateUtils {
    ...
    public static String timestamp2Normal(long time) {

        return new SimpleDateFormat(FMT_DATE_YYYYMMDD_HHMMSS).format(time);
    }
    ...
    public static Date getDateAfterHour(Date time, int hours) {
        Calendar calendar = Calendar.getInstance();
        calendar.clear();
        calendar.setTime(time);
        calendar.add(Calendar.HOUR_OF_DAY, hours); // day后
        Date newDate = new Date(calendar.getTimeInMillis());
        return newDate;
    }
