...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '天玄链区块信息表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `thanos_blockChainInfo`  (
  ...
  `is_delete` tinyint(2) NULL DEFAULT 0 COMMENT '1:删除；0:正常',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '天玄链全局信息表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `thanos_contract`  (
  ...
  `business_time` datetime(0) NULL DEFAULT NULL COMMENT '业务时间：真正发生的时间',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '浏览器-合约信息表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `thanos_contract_transaction`  (
  ...
  `update_time` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '更新时间',
  `business_time` datetime(0) NULL DEFAULT NULL COMMENT '业务时间',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_bin COMMENT = '浏览器-合约交易表' ROW_FORMAT = Dynamic PARTITION BY KEY (`contract_address`)
PARTITIONS 63
...
CREATE TABLE `thanos_evm_transaction`  (
  ...
  `block_timestamp` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '区块生成时间',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '天玄链EVM交易信息表' ROW_FORMAT = Dynamic PARTITION BY RANGE (partition_key)
PARTITIONS 2
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '天玄链交易回执信息表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '天玄链每日交易量记录表' ROW_FORMAT = Dynamic;
